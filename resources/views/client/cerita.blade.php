                ...
                @forelse($ceritas as $cerita)
                    <div class="mb-8 relative pl-8 md:pl-10 group" id="cerita-{{ $cerita->id }}">
                        ...
                        <div class="bg-brand-bg/30 border border-brand-accent/20 rounded-2xl p-5 hover:border-brand-accent/50 transition-colors relative">
                            ...
                            <button onclick="hapusCerita({{ $cerita->id }})" class="absolute top-4 right-4 text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300">
        ...
        <div class="relative inline-block w-full max-w-lg overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
            <div class="bg-white px-6 py-5">
                <div class="flex justify-between items-center mb-5">
                    ...
                    <button onclick="$('#add-cerita-modal').addClass('hidden')" class="text-gray-400 hover:text-gray-500">
            ...
            <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 rounded-b-2xl">
                <button type="button" onclick="$('#add-cerita-modal').addClass('hidden')" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">Batal</button>
    ...
    function simpanCerita() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var html = `
                    <div class="mb-8 relative pl-8 md:pl-10 group" id="cerita-${response.cerita.id}">
                        <div class="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-brand-accent ring-4 ring-white shadow-sm"></div>
                        <div class="bg-brand-bg/30 border border-brand-accent/20 rounded-2xl p-5 hover:border-brand-accent/50 transition-colors relative">
                            <button onclick="hapusCerita(${response.cerita.id})" class="absolute top-4 right-4 text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                            <span class="inline-block text-xs font-bold tracking-wider text-brand-accent uppercase mb-2 bg-brand-accent/10 px-3 py-1 rounded-full">${response.cerita.tanggal}</span>
                            <h4 class="text-lg font-bold text-brand-dark mb-2" style="font-family: 'Playfair Display', serif;">${response.cerita.judul}</h4>
                            <p class="text-sm text-gray-600 leading-relaxed">${response.cerita.isi_cerita}</p>
                        </div>
                    </div>
                `;

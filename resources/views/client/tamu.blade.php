    ...
    $(document).ready(function() {
        ...
        function loadTamu(page = 1, search = '') {
            ...
            $.get("{{ route('client.tamu.data') }}", { page: page, search: search }, function(response) {
                ...
                if(data.length === 0) {
                    html = '<tr><td colspan="5" class="px-7 py-8 text-center text-gray-500 italic border-b border-brand-accent/10">Belum ada data tamu.</td></tr>';
                } else {
                        ...
                        <tr class="hover:bg-brand-accent/5 transition-colors border-b border-brand-accent/10 last:border-0" id="tamu-${item.id}">
                            ...
                            <td class="px-7 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-3">
                                    ...
                                    <button class="text-[#25D366] hover:text-[#1EBE5D] transition-colors btn-wa" data-slug="${slug}" data-nama="${item.nama_tamu}" title="Kirim WA Manual">

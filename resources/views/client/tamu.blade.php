        ...
        <div class="bg-gradient-to-r from-brand-dark/5 to-transparent border-b border-brand-accent/15 px-4 md:px-7 py-4 md:py-5 flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 md:gap-0">
            ...
            <div class="flex flex-wrap items-center gap-2 md:gap-3 w-full md:w-auto">
                ...
                @if(!Auth::user()->invitation || !Auth::user()->invitation?->isExpired())
                    ...
                    <button onclick="$('#modal-wa').removeClass('hidden').addClass('flex');" class="flex-1 md:flex-none justify-center bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-medium transition-colors shadow-sm flex items-center gap-1.5 md:gap-2" title="Kelola Redaksi / Template Pesan WA">
                ...
                if(data.length === 0) {
                    ...
                    data.forEach(function(item) {
                        ...
                        <tr class="hover:bg-brand-accent/5 transition-colors border-b border-brand-accent/10 last:border-0" id="tamu-${item.id}">
                            ...
                            <td class="px-7 py-4 text-sm text-gray-500 font-mono">${item.no_whatsapp || item.no_wa || '-'}</td>
                            <td class="px-7 py-4">
                                ${isExpired ? 
                            ...
                            <td class="px-7 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-3">
                                    ...
                                    <button class="text-[#25D366] hover:text-[#1EBE5D] transition-colors btn-wa" data-slug="${slug}" data-nama="${item.nama_tamu}" data-wa="${item.no_whatsapp || item.no_wa || ''}" title="Kirim WA Manual">

<tr class="hover:bg-slate-50/50 transition-colors group" id="client-row-{{ $client->id }}">
    ...
    <td class="px-6 py-4">
        @if($client->invitation)
            ...
            <span class="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg border
                {{ $s === 'aktif' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : '' }}
                {{ $s === 'trial' ? 'bg-amber-50 border-amber-200 text-amber-700' : '' }}
                {{ $s === 'draft' ? 'bg-slate-50 border-slate-200 text-slate-500' : '' }}
                {{ $s === 'nonaktif' ? 'bg-red-50 border-red-200 text-red-600' : '' }}
            ">
                <span class="w-1.5 h-1.5 rounded-full 
                    {{ $s === 'aktif' ? 'bg-emerald-500' : '' }}
                    {{ $s === 'trial' ? 'bg-amber-500' : '' }}
                    {{ $s === 'draft' ? 'bg-slate-500' : '' }}
                    {{ $s === 'nonaktif' ? 'bg-red-500' : '' }}
                "></span>
                {{ $s }}
            </span>
        @else
    ...
    <td class="px-6 py-4 text-slate-500 text-xs font-medium">{{ $client->created_at->format('d M Y') }}</td>
    <td class="px-6 py-4">

...
<tr class="hover:bg-slate-50/50 transition-colors group" id="client-row-{{ $client->id }}">
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        @if($undangan && $undangan->slug)
            <a href="/{{ $undangan->slug }}" target="_blank" class="inline-flex items-center gap-1.5 text-xs font-semibold text-admin-accent hover:text-admin-accent-dark hover:underline bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200" title="Buka Undangan">
                <svg class="w-3.5 h-3.5 text-admin-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
                /{{ $undangan->slug }}
            </a>
        @else

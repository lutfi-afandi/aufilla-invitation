...
try {
    // 1. Setup Data
    $theme = Theme::firstOrCreate(['name' => 'Test Theme', 'is_active' => true], ['path' => 'test-theme', 'price' => 0]);
    ...
    // Using current() because quickRegister returns an array with [$user, $invitation] or just the user.
    // Actually quickRegister returns just the $user based on my memory.
    $user1 = is_array($user1) ? $user1[0] : $user1;
    ...
    // 4. Test Admin Creating Client with VIP (Testing Datetime Overflow)
    ...
    // Bind to container to mock validation
